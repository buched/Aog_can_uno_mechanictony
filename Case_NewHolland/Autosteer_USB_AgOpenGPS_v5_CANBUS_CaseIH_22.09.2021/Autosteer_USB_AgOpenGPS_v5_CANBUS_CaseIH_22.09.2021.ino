...
//------------------------------------
//Case / New Holland CANBUS AOGv5 - (Tony's Version)
//Only connected to Steering-Bus 
//Button Steer Engage - Via PCB or CAN (Not set for CNH Yet, Still on Valtra)
//Can Use No Steer Switch, and Turn On Via Tablet.
...
digitalWrite(engageLED,LOW);
 ...
 CAN0.init_Mask(0,1,0x00FFFFFF);       // Init first mask...  
  ...
  CAN0.init_Filt(1,1,0x0CACAA08);       // Init second filter... 
  
  CAN0.init_Mask(1,1,0x00FFFFFF);       // Init second mask... 
  CAN0.init_Filt(2,1,0x18EF1C32);       // Init third filter...
  CAN0.init_Filt(3,1,0x18EF1C32);       // Init fouth filter...
  CAN0.init_Filt(4,1,0x18EF1C32);       // Init fifth filter...
  CAN0.init_Filt(5,1,0x18EF1C32);       // Init sixth filter...
...
//--------CAN CutOut--------------------------
   ...
   if (steeringValveReady != 20 && steeringValveReady != 16){            
     ...
     if (steerConfig.InvertWAS)
      {
          steeringPosition = (estCurve - 32128 - steerSettings.wasOffset);   // 1/2 of full scale
...
CAN0.sendMsgBuf(0x0CAD08AA, 1, 8, data);  //Send CAN Curve Command
...
//**Engage Message** (Note this is still Valtra, Need to work the CaseIH out still)
  if (rxId == 0x98EF1C32){
